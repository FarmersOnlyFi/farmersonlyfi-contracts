...
contract IFO is ReentrancyGuard, Governor {
  ...
  constructor(
      ...
      uint256 _purchaseDecimal,
      IERC20 _burnToken,
      uint256 _burnDecimal,
      uint256 _tokenRatio,
      ...
      purchaseDecimal = _purchaseDecimal;
      burnToken = _burnToken;
      burnDecimal = _burnDecimal;
  ...
  function deposit(uint256 _amount) external nonReentrant {
    ...
    burnToken.safeTransferFrom(address(msg.sender), address(this), _amount);
    purchaseToken.safeTransferFrom(address(msg.sender), address(this), _amount.mul(tokenRatio).mul(purchaseDecimal).div(burnDecimal));
  ...
  function harvest() external nonReentrant {
    ...
    if (refundingTokenAmount > 0) {
      burnToken.safeTransfer(address(msg.sender), refundingTokenAmount);
      purchaseToken.safeTransfer(address(msg.sender), refundingTokenAmount.mul(tokenRatio).mul(purchaseDecimal).div(burnDecimal));
    }
  ...
  function finalWithdraw() external onlyGov {
    ...
    if (totalAmount < raisingAmount) {
        burnToken.safeTransfer(address(msg.sender), totalAmount);
        purchaseToken.safeTransfer(address(msg.sender), totalAmount.mul(tokenRatio).mul(purchaseDecimal).div(burnDecimal));
        offeringToken.safeTransfer(address(msg.sender), offeringAmount.mul(raisingAmount.sub(totalAmount)).div(raisingAmount));
    } else {
        burnToken.safeTransfer(address(msg.sender), raisingAmount);
        purchaseToken.safeTransfer(address(msg.sender), raisingAmount.mul(tokenRatio).mul(purchaseDecimal).div(burnDecimal));
    }
  }
}