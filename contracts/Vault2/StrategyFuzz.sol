...
import "../interfaces/IFuzzChef.sol";
import "../interfaces/IWETH.sol";

import "./BaseStrategyLP.sol";

contract StrategyFuzz is BaseStrategyLP {
    ...
    function _vaultDeposit(uint256 _amount) internal override {
        IFuzzChef(masterchefAddress).deposit(pid, _amount);
    }
    
    function _vaultWithdraw(uint256 _amount) internal override {
        IFuzzChef(masterchefAddress).withdraw(pid, _amount);
    }

    function earn() external override nonReentrant whenNotPaused onlyGov {
        // Harvest farm tokens
        IFuzzChef(masterchefAddress).deposit(pid, 0);
    ...
    function vaultSharesTotal() public override view returns (uint256) {
        (uint256 balance,,) = IFuzzChef(masterchefAddress).userInfo(pid, address(this));
        return balance;
    }
