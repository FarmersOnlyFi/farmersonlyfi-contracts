...
import "../interfaces/IArtemisChef.sol";
import "../interfaces/IWETH.sol";

import "./BaseStrategyLP.sol";

contract StrategyArtemis is BaseStrategyLP {
    ...
    function _vaultDeposit(uint256 _amount) internal override {
        IArtemisChef(masterchefAddress).deposit(pid, _amount, address(this));
    }
    
    function _vaultWithdraw(uint256 _amount) internal override {
        IArtemisChef(masterchefAddress).withdraw(pid, _amount);
    }

    function earn() external override nonReentrant whenNotPaused onlyGov {
        // Harvest farm tokens
        IArtemisChef(masterchefAddress).deposit(pid, 0, address(this));
    ...
    function vaultSharesTotal() public override view returns (uint256) {
        (uint256 balance,) = IArtemisChef(masterchefAddress).userInfo(pid, address(this));
        return balance;
    }
