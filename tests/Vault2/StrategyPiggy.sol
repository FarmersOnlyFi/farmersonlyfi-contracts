...
import "../interfaces/IPiggyChef.sol";
import "../interfaces/IWETH.sol";

import "./BaseStrategyLPSupportingFeeTokens.sol";

contract StrategyPiggy is BaseStrategyLPSupportingFeeTokens {
    ...
    function _vaultDeposit(uint256 _amount) internal override {
        IPiggyChef(masterchefAddress).deposit(pid, _amount, withdrawFeeAddress);
    }
    
    function _vaultWithdraw(uint256 _amount) internal override {
        IPiggyChef(masterchefAddress).withdraw(pid, _amount);
    }

    function earn() external override nonReentrant whenNotPaused onlyGov {
        // First check if we can even harvest
        if (!canHarvest()) {
            return;
        }

        // Harvest farm tokens
        IPiggyChef(masterchefAddress).deposit(pid, 0, withdrawFeeAddress);
    ...
    function distributeFees(uint256 _earnedAmt, address _earnedAddress) internal returns (uint256) {
        if (controllerFee > 0) {
            ...
            if (_earnedAddress == woneAddress) {
                ...
                _safeSwapWoneSupportingFees(fee, earnedToWonePath, controllerFeeAddress);
            }
    ...
    function canHarvest() public view returns (bool) {
        return IPiggyChef(masterchefAddress).canHarvest(pid, address(this));
    }


    function vaultSharesTotal() public override view returns (uint256) {
        (uint256 balance,,,) = IPiggyChef(masterchefAddress).userInfo(pid, address(this));
        return balance;
    }
    ...
    function _emergencyVaultWithdraw() internal override {
        IPiggyChef(masterchefAddress).withdraw(pid, vaultSharesTotal());
    }

    function emergencyPanic() external onlyGov {
        _pause();
        IPiggyChef(masterchefAddress).emergencyWithdraw(pid);
    }
